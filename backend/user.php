        ...
        if ( ($user['status'] == 1) && password_verify( $_POST['password'], $user['passwordHash'])) {
            ...
            echo json_encode( array('status'=> _createStatus(0,''), //'jwt'=>$jwt, 
                                    ...
                                    'user'=> array( 'userId'=>$user['userId']
                                                    ,'plan'=>$user['plan']
                                                    ,'terminationDatePlan'=>$user['terminationDatePlan']
                                                    ,'terminationDateAccount'=>$user['terminationDateAccount']
                                                    /*  'url2'=>$user['url2'],
...
function requestChangePasswordURL()
{
    ...
    try {
        if (  ($user = _getUserByEmail($_POST['username'])) && ($user['status'] == 1)  ) { 
            ...
            _sendPasswordResetMail($_POST['username'], BASE_URL.'reset?&t='.$token.'&m=p');   
        }
    }
    catch(error) {
        logger("unknown token");
        echo json_encode( array('status'=> _createStatus(0,error)));
        exit;
    }
